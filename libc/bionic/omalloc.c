...
wrterror(char *msg, void *p)
{
	...
	__libc_fatal_no_abort("%s %s", malloc_func, msg);
...
{
	char *p, b[64], prop[PROP_VALUE_MAX];
	int i, j;
...
void **delayed_chunks_find(struct dir_info *d, void *p) {
	size_t index;
	...
	return q == p ? &d->delayed_chunks_set[index] : NULL;
}
...
void delayed_chunks_delete(struct dir_info *d, void *p) {
	...
	slot = delayed_chunks_find(d, p);
	if (slot == NULL) {
		wrterror("missing from quarantine tracking table", p);
		return;
	}

	i = slot - d->delayed_chunks_set;
	for (;;) {
...
ofree(void *p)
{
	...
	if (sz > MALLOC_MAXCHUNK) {
		...
		if (!mopts.malloc_freenow) {
			...
			if (delayed_chunks_find(pool, p) != NULL) {
...
{
	size_t size = (bytes + PAGE_MASK) & ~PAGE_MASK;
	if (size < bytes) {
		errno = ENOMEM;
		return NULL;
	}
...
omalloc_usable_size(const void *p)
{
	...
	r = find(pool, (void *)p);
	if (r == NULL) {
		wrterror("bogus pointer in malloc_usable_size", (void *)p);
		return 0;
	}
...
o_malloc_usable_size(const void *p)
{
	...
	_MALLOC_LOCK();
	malloc_func = "malloc_usable_size():";
	if (getpool() == NULL) {
		_MALLOC_UNLOCK();
		wrterror("malloc_usable_size() called before allocation", NULL);
		return 0;
	}
